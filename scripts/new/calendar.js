...
/*
	Calendar resources are normally declared in semester.json.

	For resilience, we also try the standard semester data paths:
		college/{semester}/data/timetable.json
		college/{semester}/data/holidays.json
		college/{semester}/data/class-log.json

	This means the calendar does not silently lose class/note counts merely
	because an older semester.json is still present locally.
*/
async function loadCalendarResources(semesterId, declared) {
	...
	console.info('Calendar resources:', {
		semesterId,
		timetableLoaded: Boolean(timetable),
		holidaysLoaded: Boolean(holidays),
		classLogDates: Object.keys(classLog ?? {}),
	});
...
function configurePage(state) {
	...
	const timetablePanel = document.getElementById('weekly-timetable-panel');
	const timetableCopy = document.getElementById('weekly-timetable-copy');

	if (state.timetable) {
		if (timetablePanel) timetablePanel.hidden = false;
		...
		if (timetableCopy) {
			...
			timetableCopy.textContent =
				`Official class timetable effective ${from}${to}.`;
		}
	} else if (timetablePanel) {
...
function renderCalendar(state) {
	...
	title.textContent = state.currentMonth.toLocaleDateString(
		'en-IN',
		{
			month: 'long',
			year: 'numeric',
...
function createDayCell(date, state) {
	...
	if (holiday.isHoliday) button.classList.add('is-holiday');
	if (academic.events.length) button.classList.add('has-academic-event');
	if (notes.length) button.classList.add('has-notes');
	if (scheduled.length) button.classList.add('has-schedule');
	...
	/*
		...
		A single day can show academic event + holiday + classes + notes.
	*/

	if (academic.events.length) {
		tags.push(`
			<span class="calendar-day-tag academic-tag">
	...
	if (scheduled.length) {
		const count = countScheduledPeriods(scheduled);
		...
		<div class="calendar-day-top">
			...
			${
				notes.length
					? `
						<span
							class="calendar-note-marker"
							title="Class notes added"
						></span>
					`
					: ''
			}
...
function countScheduledPeriods(schedule) {
	return schedule.reduce(
		(total, item) => total + (item.periods?.length ?? 1),
		0,
	);
}
...
function renderDayDetail(date, state) {
	...
	if (
		!academic.events.length &&
		!notes.length &&
		!scheduled.length
	) {
		html = `
			<p class="calendar-empty">
				No academic events, scheduled classes or
...
function getAcademicInfo(
	...
	const semester =
		academicCalendar.semesters?.[semesterId];
	...
	return {
		...
		suppressRegularClasses:
			events.some(
				event => event.suppressRegularClasses,
			) ||
			!isInstructionalDate,
	};
}
...
function renderScheduleItem(item, timetable) {
	...
	const start =
		timetable?.periods?.find(
			period => period.id === firstPeriod,
		)?.start || '';

	const end =
		timetable?.periods?.find(
			period => period.id === lastPeriod,
		)?.end || '';
...
function renderAcademicEvent(event) {
	...
	return `
		<article class="academic-event-item">
			<div class="academic-event-title">
				${escapeHtml(event.title)}
			</div>

			<div class="academic-event-date">
				${escapeHtml(dateText)}
			</div>
		</article>
	`;
...
function setupTimetableModal(state) {
	...
	const openButton =
		document.getElementById('show-weekly-timetable');
	...
	const mount =
		document.getElementById('weekly-timetable-table');
...
function buildWeeklyTimetable(timetable) {
	if (!timetable?.periods || !timetable?.days) {
		return `
			<p class="calendar-empty">
				Weekly timetable is not available yet.
			</p>
		`;
	}
