...
- [ ] For every independently verified source/textbook content discrepancy, use the single shared project-wide MAC golden state across all subjects and semesters: highlight the question/example number or heading in solid amber/gold, highlight only the affected subpart label(s) when multipart, and place the entire discrepancy explanation inside the matching amber/gold note box. State exactly what conflicts and whether the retained/corrected solution follows the printed source or an independently verified correction. Use this for meaning-bearing discrepancies (including mathematics, figure values, labels, topology, or question/solution inconsistencies), never for grammar/editorial clean-up.
...
- [ ] Reference each in-scope textbook problem/exercise set on the theory page at the same logical point where it appears in the book, linking to the exact question-group anchor.
