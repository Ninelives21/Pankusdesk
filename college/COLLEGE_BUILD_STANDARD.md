...
21. **Question-set transitions must be unmistakable.** On textbook-question and other grouped practice pages, every new problem/exercise set must begin with a strong shared section banner that is visually distinct from the question cards. Keep the set title, book/page context, explanatory note and question count visible in that banner; do not remove useful orientation text merely to make the heading bolder. Use the shared dark-primary question-group treatment across subjects.
