...
## 22. Shared study UI — project-wide design lock

This is a project-wide invariant across every subject, unit, semester and future college build.

- Common study UI must be implemented once in `scripts/new/study-ui.js` and `styles/new/study-ui.css`. Subject code may provide data/layout context, but must not fork the visual/behavioural implementation of common accordions, MathJax/LaTeX rendering, worked-example question/solution presentation, study-tip boxes or baseline study figures.
- Worked examples use one shared accordion pattern: the example title and **complete question are visible in the closed accordion header**; the `+` opens that same card; the opened body begins with the non-clickable label **Solution** and contains the complete working. Never add a separate clickable `Solution` row/button beneath the question.
...
- Model-added explanatory material that is not part of the source/class transcription must be visibly separated from source text. Short advisory/context notes use the shared blue `PankusDesk tip` treatment. **Blue PankusDesk tips are collapsible `<details>` dropdowns by default** so explanatory additions do not bloat study/class-note pages; the closed header shows the tip label/title and a `+`, and the body opens on demand. Optional detailed simplified teaching uses the shared collapsible `Simple terms` explainer rendered by `PankuStudyUI.renderExplainer()`.
...
- **SIMPLER is deliberately hand-holding, not merely shorter.** Write it for a student returning with little or no remembered context. Re-establish the visual/semantic clue before using it: for example, say that a diamond identifies a dependent source, an arrow inside a source identifies a current source, `+/-` marks define voltage polarity, uninterrupted wire/junction dots establish node identity, the same two end nodes establish parallel branches, and an unbranched end-to-end path establishes series current. Do not assume that the reader will remember why a classification or sign is valid merely because it was taught earlier.
- In a SIMPLER worked example, explicitly connect each important equation to what can actually be seen in the figure: which arrowhead shows entering/leaving current, which polarity is crossed, why a branch is series/parallel, why a source is independent/dependent, and what a negative result means. Prefer phrases such as “we know this because…” when a visual convention is doing real reasoning work. Avoid unexplained jumps such as “obviously”, “clearly”, or “by inspection” unless the visual evidence is immediately stated.
